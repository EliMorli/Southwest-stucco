...
export default function GeneralContractorsPage() {
  ...
  return (
    <>
      ...
      <section className="bg-mesa-fade py-16 px-[5vw]">
        <div className="max-w-[1200px] mx-auto">
          <div className="grid md:grid-cols-[1fr_2fr] gap-10 items-start">
            <div>
              <span className="font-condensed text-[0.7rem] tracking-[0.42em] uppercase text-sun/80 font-bold block mb-3">
                Capabilities
              </span>
              <h2 className="font-display text-[clamp(1.8rem,3.4vw,2.6rem)] leading-[0.95] text-bone mb-4">
                The numbers your{' '}
                <span className="font-serif italic text-sun">project manager</span>{' '}
                actually needs.
              </h2>
              <p className="text-bone/65 leading-[1.75] text-[0.96rem]">
                Screenshot this section and forward it to your superintendent. Or call{' '}
                <a
                  className="text-sun hover:text-clay border-b border-sun/40 no-underline"
            ...
            <dl className="grid sm:grid-cols-2 gap-[1px] bg-sun/15 rounded-sm overflow-hidden">
              {capabilities.map((c) => (
                <div
                  key={c.label}
                  className="bg-mesa-100 px-5 py-4 flex flex-col justify-between"
                >
                  <dt className="font-condensed text-[0.62rem] tracking-[0.28em] uppercase text-sun/75 font-bold">
                    {c.label}
                  </dt>
                  <dd className="m-0 font-bold text-bone text-[0.95rem] leading-snug mt-1">
                    {c.value}
                  </dd>
                </div>
              ))}
      ...
      <section className="plaster-dark py-24 px-[5vw]">
        <div className="max-w-[1300px] mx-auto">
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-[1px] bg-sun/10 rounded-sm overflow-hidden">
            {gcAudienceFacts.map((f) => (
              <article key={f.title} className="bg-mesa-100/80 p-7 backdrop-blur-sm">
                <span className="text-[1.8rem]" aria-hidden>{f.icon}</span>
                <h3 className="font-display text-[1.45rem] tracking-wide text-bone mt-3 mb-2">
                  {f.title}
                </h3>
                <p className="text-[0.92rem] text-bone/65 leading-[1.7]">{f.body}</p>
              </article>
            ))}
          </div>
        </div>
        ...
        <div className="max-w-[1100px] mx-auto">
          ...
          <ol className="grid md:grid-cols-2 gap-x-10 gap-y-8 list-none p-0 m-0">
            {gcProcess.map((p, i) => (
              <li
                key={p.title}
                className="grid grid-cols-[auto_1fr] gap-x-5 pt-6 border-t border-sun/15"
              >
                <span className="font-serif italic text-[2.4rem] leading-none text-clay select-none">
                  {String(i + 1).padStart(2, '0')}
                </span>
                <div>
                  <h3 className="font-display text-[1.4rem] tracking-wide text-bone mb-2 leading-tight">
                    {p.title}
                  </h3>
                  <p className="text-[0.94rem] text-bone/70 leading-[1.75]">{p.body}</p>
                </div>
              </li>
            ))}
          </ol>
      ...
      <section className="plaster-dark py-24 px-[5vw]">
        <div className="max-w-[1300px] mx-auto">
          ...
          <div className="grid lg:grid-cols-[1.2fr_1fr] gap-10 items-start">
            <ul className="list-none p-0 m-0 grid gap-[1px] bg-sun/10 rounded-sm overflow-hidden">
              {sampleProjects.map((p) => (
                <li key={p.name} className="bg-mesa-100 p-6 md:p-7">
                  <h3 className="font-display text-[1.35rem] tracking-wide text-bone mb-1">
                    {p.name}
                  </h3>
            ...
            <aside className="bg-mesa-100/80 border border-sun/15 rounded-sm p-7 backdrop-blur-sm lg:sticky lg:top-24">
              <h3 className="font-display text-[1.5rem] tracking-wide text-bone mb-2">
                References on request
              </h3>
              <p className="text-bone/65 leading-[1.7] text-[0.94rem] mb-5">
        ...
        <div className="max-w-[1300px] mx-auto">
          <header className="grid md:grid-cols-[auto_1fr_auto] items-end gap-x-6 mb-12">
            ...
            <div>
              ...
              <div className="thin-rule max-w-[260px]" aria-hidden />
              <h2 className="mt-4">
                <span className="block font-display text-[clamp(2rem,4vw,3.6rem)] leading-[0.95] text-bone">
                  Field notes
                </span>
                <span className="block font-serif italic text-[clamp(2rem,4.2vw,3.8rem)] leading-[0.95] -mt-1 text-sun">
                  from a 30-year sub.
                </span>
              </h2>
            </div>
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-5">
            {gcPosts.map((p) => (
              <Link
                key={p.slug}
                href={`/blog/${p.slug}`}
                className="group bg-mesa-100/60 border border-sun/12 rounded-sm overflow-hidden no-underline transition-all hover:border-clay/40 hover:-translate-y-1 flex flex-col"
              >
