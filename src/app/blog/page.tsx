        ...
        <div className="max-w-[1300px] mx-auto grid md:grid-cols-2 lg:grid-cols-3 gap-6">
          {blogPosts.map((p) => (
            <Link
              key={p.slug}
              href={`/blog/${p.slug}`}
              className="group bg-bone border border-adobe/15 rounded-2xl overflow-hidden no-underline transition-all hover:border-terracotta/40 hover:-translate-y-1 flex flex-col"
            >
